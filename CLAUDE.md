...
- **`analyzer/`** — Python 3.11. Workers (`ANALYZER_WORKERS`, default 2) call Claude (`CLAUDE_MODEL`, default `claude-sonnet-4-5`) with the prompt in `analyzer/src/prompt.py`; `validator.py` enforces the JSON shape of the 45+-field output before writing back. `knowledge_base.py` produces the Dapta export.
